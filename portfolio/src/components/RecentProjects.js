  ...
  render() {
    if(this.state.loading===true){
      ...
      const projectView= myList.map(m => {
        return (
          <Col key={m.id} lg={4} md={6} sm={12} className="py-5">
            <Card className="projectCard">
              <Card.Img className="projectImg" variant="top" src={m.project_img_one} />
              <Card.Body>
      ...
      return (
        <Fragment>
          <Container className="text-center ">
            <h1 className="text-center my-5 headingTitle">Recent Projects</h1>
            <Row>{projectView}</Row>
          </Container>
        </Fragment>
      );
